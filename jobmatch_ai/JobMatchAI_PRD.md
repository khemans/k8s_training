...
### Next Step: Local Kubernetes Cluster

The current deployment relies on hosted services (Vercel, Railway, Supabase, RapidAPI JSearch) which have **free-tier and quota limitations** that constrain realistic load testing and experimentation.

**Goal:** Stand up a **locally hosted Kubernetes cluster** that can run the full JobMatch AI stack end-to-end for more controlled, cost-free testing.

**Initial scope (non-goals included):**
- Reuse existing Docker images and `docker-compose` definitions as the basis for Kubernetes manifests (no app-level rewrites required).
- Target a single-node local cluster (e.g., kind, k3d, or minikube) running on the developer machine.
- Keep all required secrets and configuration in Kubernetes `Secret`/`ConfigMap` objects wired from `.env`-style files.
- Mirror the existing topology: frontend, backend API, ARQ worker, Postgres, Redis.
- Maintain compatibility with the current cloud deployment (no breaking changes to app configuration or env vars).

Cloud migration of this Kubernetes setup (e.g., to managed K8s) is **explicitly out of scope** for this next step and can be considered in a later phase once local Kubernetes is stable.
